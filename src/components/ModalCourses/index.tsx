...
export function ModalCourses({ isOpen, onRequestClose, infoClasses, course }: ModalNewClassesProps) {
    ...
    const handleMyCourse = async () => {
        ...
        await apiClient.put('/users/update', { courseId, userId }).then(response => {
            setRegistered(true)
            onRequestClose()
        }).catch((err) => {
    ...
    return (
        ...
        >
            ...
            <div className={styles.content}>
                    ...
                    <div className={styles.class}>
                        ...
                        <div className={styles.boxScrollClass}>
                            {infoClasses.map(item => (
                                <div key={item.id} className={styles.infoClass}>
                ...
                <div key={course.id} className={styles.contentInfoCourse}>
                ...
                <div key={course.id} className={styles.description}>
                    <h1>Descrição</h1>
                    <text>{course.description}</text>
                </div>

                <div key={course.id} className={styles.boxTeacher}>
                    <img alt={course.title} src={`http://localhost:3333/files/${course.teacherphoto}`} />
                ...
                {!registered && <button className={styles.button} title="iniciar curso" onClick={() => handleMyCourse()}>Iniciar Curso</button>}
            </div>
        </Modal>
    )
}